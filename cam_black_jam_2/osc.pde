...
void sendStart_local( int x, int y)
{
  OscMessage msg = new OscMessage("/sound/start");
  msg.add(x).add(y);

  NetAddress addr = new NetAddress(localhost, remoteport);
  osc.send(msg, addr);  
}

void sendStop_local( int x, int y)
{
  OscMessage msg = new OscMessage("/sound/stop");
  msg.add(x).add(y);  
